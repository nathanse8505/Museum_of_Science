...
def main():
    ...
    while running:
        ...
        ##################################  camera working ###################################
            ...
            img,camera_working,camera_on = take_new_pic(cap, camera_working)
            if not camera_working:
                continue
            #saving picture
            ...
            if found_arduino and byte_list is not None and black_percentage > empty_image_threshold:
                ...
                VALID = reset_buffer_arduino(arduino,byte_list,log)
                if not VALID:
                    continue
                ...
                if empty_captures_in_a_row >= empty_captures_before_idle:  # if the empty captures counter is more than 'empty_captures_before_idle', go to idle mode
                    ...
                    byte_list, _ = process_and_save_image(in_path_idle,out_path,log)  # process the sample image
