...
def find_arduino_port(logger=None):
    ...
    for port in ports:
        ...
        # print(f"port: {port}")
        if "Arduino" in port.description or "ttyUSB" in port.device or "ttyACM" in port.device or "COM6" in port.device:
...
def parse_data(raw_data, logger=None):
    ...
    try:
        ...
        distance = max(min(float(data[0]), MAX_DISTANCE),MIN_DISTANCE)

        if hasattr(parse_data, "already_sent_error"):
            parse_data.already_sent_error = False
            
        return distance, PARSE_VALID
