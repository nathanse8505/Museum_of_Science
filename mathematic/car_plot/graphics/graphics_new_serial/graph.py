...
class Graph:
    ...
    def __init__(self, screen, function, x_range, y_range, sub_surface, title, color=GRAPH_COLOR, width=GRAPH_LINE_WIDTH, step=1):
        ...
        self.width = width
        self.step = step
        self.cached_surface = None  # cache
    ...
    def render_surface(self):
        ...
        graph_surface = pygame.Surface((width, height),
                                       pygame.SRCALPHA)  # Create a new surface with size of the sub-surface
        ...
        pygame.draw.lines(graph_surface, self.color, False, screen_points,
                          self.width)  # Draw the line on the graph surface
        ...
        graph_surface.blit(title_surface, title_rect)
    ...
    def draw(self):
        ...
        if self.cached_surface is None:
            self.cached_surface = self.render_surface()

        self.screen.blit(self.cached_surface, (pos_x, pos_y))
