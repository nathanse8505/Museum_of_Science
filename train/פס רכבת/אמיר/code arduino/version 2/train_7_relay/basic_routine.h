...
const int trainDetectionSignal = LOW;   // capteur actif à LOW (INPUT_PULLUP)
const int barrierCloseSignal   = LOW;   // close = LOW (relais NC)
...
bool isBarrierClosed(int segmentIndex);
...
bool isBarrierClosed(int segmentIndex) {
  return (digitalRead(barrierPins[segmentIndex]) == barrierCloseSignal);
}
...
void manualRelaySelector() {
  ...
  while (true) {
    ...
    if (nextButtonPressed()) {
      ...
      closeBarrier(selected); // CLOSE immédiat du relais sélectionné
    }
    ...
    if (okClickCount == 1 && (millis() - lastOkClickMs) > DOUBLE_CLICK_MS) {
      ...
      if (isBarrierClosed(selected)) {
        openBarrier(selected);
      } else {
        closeBarrier(selected);
      }
      okClickCount = 0;
    }
