...
def parse_args():
    ...
    parser.add_argument("--interval", choices=["day", "hour"], default="day", help="Graph granularity: 'day' or 'hour'. Default is 'day'.")
...
def analyze_logs(files, start_dt, end_dt, interval):
    ring_jumped_counter = defaultdict(int)
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    if "Ring jumped!" in line:
                        ring_jumped_counter[time_key] += 1
                        any_data_found = True
    ...
    return {
        "Ring jumped!": ring_jumped_counter,
...
def write_summary(data_dict, interval, start_dt, end_dt):
    ...
    ring_total = total(data_dict["Ring jumped!"])
    ...
    filename = f"jumping_ring_summary_{start_fmt}_{end_fmt}.txt"
    ...
    with open(filename, "w") as f:
        ...
        f.write(f"Total ring jumps: {ring_total}\n")
        ...
        f.write(f"Average ring jumps per {interval}: {ring_avg:.2f}\n")
...
def plot_and_save_counts(data_dict, interval, start_dt, end_dt):
    ...
    start_fmt = sanitize_for_filename(start_dt.strftime("%Y-%m-%d_%H-%M-%S"))
    end_fmt = sanitize_for_filename(end_dt.strftime("%Y-%m-%d_%H-%M-%S"))
    filename = f"jumping_ring_plot_{start_fmt}_{end_fmt}.png"
...
def confirm_large_hour_interval(start_dt, end_dt, interval):
    if interval == "hour":
        ...
        if delta_days >= 6:
            print(f"⚠️  WARNING: Time range is {delta_days} days but interval is set to 'hour'. This may produce a large and cluttered graph.")
...
def main():
    ...
    confirm_large_hour_interval(start_dt, end_dt, args.interval)

    data_dict = analyze_logs(args.files, start_dt, end_dt, args.interval)
    if not data_dict:
        print("No data found in the specified time range. No graph was generated.")
        sys.exit(0)

    plot_and_save_counts({k: v for k, v in data_dict.items() if "Timestamp" not in k}, args.interval, start_dt, end_dt)
    write_summary(data_dict, args.interval, start_dt, end_dt)
