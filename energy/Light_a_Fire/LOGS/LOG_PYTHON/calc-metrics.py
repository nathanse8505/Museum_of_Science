...
def analyze_logs(files, start_dt, end_dt, interval):
    fire_ignite_counter = defaultdict(int)
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    if "Peak temperature reached" in line:
                        fire_ignite_counter[time_key] += 1
                        any_data_found = True
    ...
    return {
        "The fire ignites": fire_ignite_counter,
...
def write_summary(data_dict, interval, start_dt, end_dt):
    ...
    ring_total = total(data_dict["The fire ignites"])
    ...
    filename = f"Light_a_fire_summary_{start_fmt}_{end_fmt}.txt"
    ...
    with open(filename, "w") as f:
        ...
        f.write(f"Total fire ignite: {ring_total}\n")
        ...
        f.write(f"Average fire ignite per {interval}: {ring_avg:.2f}\n")
...
def plot_and_save_counts(data_dict, interval, start_dt, end_dt):
    ...
    start_fmt = sanitize_for_filename(start_dt.strftime("%Y-%m-%d_%H-%M-%S"))
    end_fmt = sanitize_for_filename(end_dt.strftime("%Y-%m-%d_%H-%M-%S"))
    filename = f"Light_a_Fire_plot_{start_fmt}_{end_fmt}.png"
