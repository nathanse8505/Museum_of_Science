...
def analyze_logs(files, start_dt, end_dt, interval):
    ball_launch_counter = defaultdict(int)
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                try:
                    ...
                    if "your horsepower is" in line:
                        ball_launch_counter[time_key] += 1
                        any_data_found = True
    ...
    return {
        "HORSEPOWER_COUNTER": ball_launch_counter,
...
def write_summary(data_dict, interval, start_dt, end_dt):
    ...
    ring_total = total(data_dict["HORSEPOWER_COUNTER"])
    ...
    filename = f"Horsepower_summary_{start_fmt}_{end_fmt}.txt"
    ...
    with open(filename, "w") as f:
        ...
        f.write(f"Total ball launch: {ring_total}\n")
        ...
        f.write(f"Average ball launch per {interval}: {ring_avg:.2f}\n")
...
def plot_and_save_counts(data_dict, interval, start_dt, end_dt):
    ...
    start_fmt = sanitize_for_filename(start_dt.strftime("%Y-%m-%d_%H-%M-%S"))
    end_fmt = sanitize_for_filename(end_dt.strftime("%Y-%m-%d_%H-%M-%S"))
    filename = f"Horsepower_plot_{start_fmt}_{end_fmt}.png"
