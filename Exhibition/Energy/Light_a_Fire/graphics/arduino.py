...
def find_arduino_port(logger=None):
    ...
    for port in ports:
        ...
        # print(f"port: {port}")
        if "Arduino" in port.description or "ttyUSB" in port.device or "ttyACM" in port.device or "COM4" in port.device: # Support for Windows:
...
def parse_data(raw_data, logger=None):
    ...
    try:
        ...
        return Temperature, Sensor_analogread, PARSE_VALID
        ...
        return 0, 0, PARSE_ERROR  # default values for Pressure Value, Sensor_analogread, language
