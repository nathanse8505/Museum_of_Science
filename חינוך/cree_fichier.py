...
def create_or_update_docx(doc_path: Path, folder_name: str):
    # Crée ou remplace le fichier Word
    doc = Document()
...
def process_parent(parent: Path):
    ...
    for child in sorted(parent.iterdir()):
        ...
        # Crée le dossier תמונות
        images_dir = child / HEB_IMAGES_DIR
        images_dir.mkdir(parents=True, exist_ok=True)

        # Crée le fichier מפרט טכני.docx
        doc_path = child / SPEC_FILENAME
        create_or_update_docx(doc_path, folder_name=child.name)
        count += 1
